...
<?php 
$con = mysqli_connect("localhost","root","","gadgetblast");
...
function cart(){
  ...
  if(isset($_GET['add_cart'])){
	  ...
	  if(mysqli_num_rows($run_check_pro) > 0){
		...
		echo "<script>window.open('here_products.php','_self')</script>";
		...
		while($row_pro = mysqli_fetch_array($run_pro)){
		  ...
		  echo "
		      <div id='single_product'>
			    <h3>$pro_title</h3>
				<img src='admin/product_images/$pro_image' width='300' height='250' />
				
				<p><b> Price: tk $pro_price </b></p>
				
				<a href='details.php?pro_id=$pro_id'>Details</a>
				
				<a href='here_products.php?add_cart=$pro_id'>
